...
from dhutils import grouper, sliding_window, count_sliding_window, camel_to_snake
from augmentpipe_origin import RandomAugmentPipe, RandomAugmentPipePaper, RandomAugmentPipeReversed
import augmentpipe_origin
...
class HSI_CR(nn.Module):
    ...
    def __init__(self, num_band,num_class,aug_name,augs,augargs,patch_size=-1,dropout=0.5,device=torch.device('cuda:0')):
        super(HSI_CR, self).__init__()
        self.num_class = num_class
        self.dropout = dropout
        
        if aug_name == 'RandomAugmentPipeADA':
            aug_class = getattr(augmentpipe_origin,aug_name)
            augs = {k:1 for k in augs}
            self.augment = aug_class(**augs)
        elif aug_name == 'RandomAugmentPipe':
            aug_class = getattr(augmentpipe_origin,aug_name)
            self.augment = aug_class(num_band,patch_size,device,augs,augargs)
        elif aug_name == 'RandAugment':
            aug_class = getattr(augmentpipe_origin,aug_name)
            self.augment = aug_class()
        else:
            self.augment = None
        ...
        self.part1 = nn.Sequential(
                ...
                nn.BatchNorm3d(12),
                nn.ReLU())
        self.part2 = nn.Conv3d(12,50,(1,1,int(math.ceil(num_band/2-14))),(1,1,1))
        
        self.part3 = nn.Sequential(nn.BatchNorm3d(50),
                nn.ReLU())
        
        self.part4 = nn.Sequential(Residual(1,12),
                Residual(12,12),
    ...
    def forward(self, input):
        ...
        res2 = self.part2(res1)
        res3 = self.part3(res2)
        res4 = torch.transpose(res3,1,4)
        res5 = self.part4(res4)
        return res5
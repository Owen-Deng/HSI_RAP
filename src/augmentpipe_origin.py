...
class AugmentPipe(torch.nn.Module):
    def __init__(self,
        xflip=0, yflip=0, transpose =0,srotateinde=0, cutout_2 = 0,rotate90=0, xint=0, xint_max=0.125,shearx=0,sheary=0,shear_max=0.2,
        scale=0, rotate=0, aniso=0, xfrac=0, scale_std=0.2, rotate_max=1, aniso_std=0.2, xfrac_std=0.125,
        noise=0, cutout=0, noise_std=0.001, cutout_size=0.5,srotate=0,device=None
    ):
        super().__init__()
        self.register_buffer('p', misc.constant(1,device=device))       # Overall multiplier for augmentation probability
        # Pixel blitting.
        self.xflip            = float(xflip)            # Probability multiplier for x-flip.
        self.yflip            = float(yflip)            # Probability multiplier for x-flip.
        self.transpose            = float(transpose)            # Probability multiplier for x-flip.
        self.srotateinde            = float(srotateinde)            # Probability multiplier for x-flip.
        self.cutout_2            = float(cutout_2)            # Probability multiplier for x-flip.
        self.rotate90         = float(rotate90)         # Probability multiplier for 90 degree rotations.
        self.xint             = float(xint)             # Probability multiplier for integer translation.
        self.xint_max         = float(xint_max)         # Range of integer translation, relative to image dimensions.
        self.shear_max      = float(shear_max)
        self.shearx          = float(shearx)
        self.sheary          = float(sheary)
        self.srotate        = float(srotate)
        ...
        #self.register_buffer('Hz_geom',None)
        self.register_buffer('Hz_geom',upfirdn2d.setup_filter(wavelets['sym6'],device=device))
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if self.transpose > 0: 
            i = torch.floor(torch.rand([batch_size,2], device=device) * 2)
            i[:,1] = torch.where(i[:,0] == 0,1,0)
            i = torch.where(torch.rand([batch_size,1], device=device) < self.transpose * self.p, i, torch.zeros_like(i))
            if debug_percentile is not None:
                i = torch.full_like(i, torch.floor(debug_percentile * 2))
                i[:,1] = torch.where(i[:,0] == 0,1,0)
            ...
            #     newimg = torch.transpose(images[idx],1,2)
            #     plt.subplot(4,10,idx+2)
            #     plt.imshow(newimg.permute(1,2,0).int().cpu().numpy())
        ...
        if self.shearx > 0:
            t = (torch.rand(batch_size, device=device) * 2 - 1) * self.shear_max
            t = torch.where(torch.rand(batch_size, device=device) < self.shearx * self.p, t, torch.zeros_like(t))
            if debug_percentile is not None:
                t = torch.full_like(t, (debug_percentile * 2 - 1) * self.shear_max)
            G_inv = G_inv @ shear(t,0)
            
        if self.sheary > 0:
            t = (torch.rand(batch_size, device=device) * 2 - 1) * self.shear_max
            t = torch.where(torch.rand(batch_size, device=device) < self.sheary * self.p, t, torch.zeros_like(t))
            if debug_percentile is not None:
                t = torch.full_like(t, (debug_percentile * 2 - 1) * self.shear_max)
        ...
        if G_inv is not I_3:
            ...
            # images = torch.nn.functional.grid_sample(input=images, grid=grid, mode='bilinear', padding_mode='reflection', align_corners=False)
            # Calculate padding.
            cx = (width - 1) / 2
            cy = (height - 1) / 2
        ...
        if self.noise > 0:
            ...
            minmax = torch.where(torch.rand([batch_size, 1,height, width], device=device) < 0.5 ,min_val,max_val)
            minmax = minmax.repeat(1,num_channels,1,1)
            rand_map = torch.rand([batch_size, 1,height, width], device=device)
            rand_map = rand_map.repeat(1,num_channels,1,1)
            images = torch.where(rand_map < self.noise *self.noise_std *self.p,minmax,images)
            
            # torch.where(torch.rand([batch_size, height, width], device=device) < self.noise *self.noise_std * self.p,1,torch.zeros([batch_size, height, width],device=device))
            # images[:,:]            
            # images = torch.where(torch.rand([batch_size, height, width], device=device) < self.noise *self.noise_std *self.p,min,images)
        ...
        if self.srotate > 0:
            i = torch.floor(torch.rand(batch_size, device=device) * 24)
            i = torch.where(torch.rand(batch_size, device=device) < self.srotate * self.p, i, torch.zeros_like(i))
            num_circles = images.shape[-1] // 2
            for idx in range(batch_size):
                if i[idx] == 0:
                    continue
                srotate(images[idx],i[idx]*15,device,num_circles)
        
        
        if self.srotateinde > 0:
            i = torch.floor(torch.rand(batch_size, device=device) * 24)
            i = torch.where(torch.rand(batch_size, device=device) < self.srotateinde * self.p, i, torch.zeros_like(i))
            i2 = torch.floor(torch.rand(batch_size, device=device) * 2)
            i2 = torch.where(torch.rand(batch_size, device=device) < self.transpose * self.p, i2, torch.zeros_like(i2))
            if debug_percentile is not None:
                i = torch.full_like(i, torch.floor(debug_percentile * 24))
                i2 = torch.full_like(i2, torch.floor(debug_percentile * 2))
            num_circles = images.shape[-1] // 2
            for idx in range(batch_size):
                srotateinde(images[idx],i[idx]*15,device,num_circles,num_channels,1 - 2 * i2[idx])
                
        
        
        # if self.cutout_2 > 0:
        #     i2 = torch.floor(torch.rand([batch_size], device=device) * 2)
        #     i2 = torch.where(torch.rand([batch_size], device=device) < self.cutout_2 * self.p, i2, torch.zeros_like(i2))
            
        #     x = images.permute(0,2,3,1).cpu().numpy()
        #     num_samples = x.shape[0]
        #     patch_size = x.shape[-2]
        #     num_rings = int((patch_size-1) / 2)
        #     num_bands = x.shape[-1]
        #     for i in range(num_samples):
        #         if i2[i] == 0:
        #             continue
        #         ring_idx = np.random.randint(0,num_rings)
        #         band_start = np.random.randint(0,num_bands)
        #         band_end = np.random.randint(band_start,num_bands+1)
        #         new_sample = utils.ring_cut_out_3d(x[i],ring_idx,band_start,band_end,0)

                
        #         while True:
        #             ring_idx2 = np.random.randint(0,num_rings)
        #             if ring_idx2 != ring_idx:
        #                 break
                    
        #         band_start = np.random.randint(0,num_bands)
        #         band_end = np.random.randint(band_start,num_bands+1)
        #         new_sample = utils.ring_cut_out_3d(new_sample,ring_idx2,band_start,band_end,0)
        #         x[i] = new_sample.reshape((1,*new_sample.shape))

        #     images = torch.from_numpy(x).to(images.device).permute(0,3,1,2)
...
def srotate(image,angle,device,num_circles):
    angle = int(angle)
    if angle in srotate_dict:
        image[:,srotate_dict[angle][0],srotate_dict[angle][1]] = image[:,srotate_dict[angle][2],srotate_dict[angle][3]]
        return
    ...
    srotate_dict[angle] = [idx,idy,r_idx,r_idy]
